...
This script was used for models of log jacobian determinants (extracted directly with ANTs) and modulated gray and wite matter (the ANTs registration transform was applied to the fsl_anat segmented files, and they were then multiplied by the jacobian determinants).
...
* Transfer or extend the models : preparing the new data (as described above), running the models either in a cluster with the runner or on a local machine, getting subject-level Zscore brain maps
