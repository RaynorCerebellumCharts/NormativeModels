...
This repository includes all code used for the estimation of voxelwise normative models of volumetry across the whole brain, and for the application of these estimated models to samples of interest (subject born preterm and patients with spinocerebellar ataxia). The corresponding manuscript is available at (https://www.biorxiv.org/content/10.64898/2026.03.27.714738v1-0) and models are available at (https://surfdrive.surf.nl/s/Mb6mZyFmJeCaPcZ). We cannot openly share participant IDs in the train and test set as they are not all public, but in case users would like to extract normative deviations scores from datasets already included in our aggregate sample (e.g. ABCD), they are  welcome to reach out for collaboration. We estimated additional models with reverse train-test splits, such that collaborators could extract such deviation scores in a principled manner for their own analyses by leveraging both model iterations. 
